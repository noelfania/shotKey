...
- 플릭 패드 5열×5행: 좌·우(및 탁음 `^^` 자리)·최하단(우끝 제외)은 iOS 기능키 스켈레톤(문자·입력 없음), 중앙 3열이 실키
- 우하단 키(`restart`)는 **Restart** 버튼
- 출제 1글자(프롬프트 스테이지 **가로·세로 중심**) + 오른쪽 다음 3글자 미리보기(동일 font-size, opacity만 단계 감소)
- 집중 게이지(PC와 동일 decay·보상·MISS −15), **출제 문자 바로 아래** 슬림 바, 첫 입력 전엔 감소 없음
- 반응속도 판정 `PERFECT` / `GOOD` / `OK` / `MISS` / `TIME OUT` + 효과음·프롬프트 애니
- 게이지 0 → `TIME OUT` 후 입력 중단 → Restart로 재개
- MISS 시 상·하 사선 경고(문자 비가림), 600ms 입력 잠금
- 스크립트별 누적 문자 통계·위험도·`Most missed` 상위 5 랭킹(출제와 플릭 패드 **사이**)
- 플릭 키 **개별 글자** 위험 색상(키 전체가 아님, 위험도 `0~0.95`)
- 스크립트 토글(히라/카타), 효과음 on/off (효과음 설정은 새로고침 시 기본 on; Sound ON 시 확인음으로 Web Audio unlock)
- 모바일 뷰포트 고정(스크롤 없음): 상단(BuildMeta·스크립트/사운드·출제 `34dvh`) → 중간 여백 → 하단 도크(Most missed + 플릭 패드 `33.333dvh`)
...
- **iOS/WebKit 효과음 불안정·무음** — Sound ON·unlock 완화 후에도 기기/브라우저에 따라 들리지 않을 수 있다. 추적: [known-issues.md](../planning/known-issues.md)
